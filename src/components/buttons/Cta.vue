...
export default defineComponent({
  ...
  setup(props) {
    ...
    let minWidth = props.msg ? '14rem' : '3.5rem';
    let spriteMargin = props.msg ? '5%' : '0';
    let pMargin = props.msg && props.sprite ? '15%' : '0';
    console.log(props.msg && props.sprite ? '15%' : '0');

    if (message) {
      message = props.msg.charAt(0).toUpperCase() + props.msg.slice(1);
    }
    return { bgColor: props.color, message, minWidth, spriteMargin, pMargin };
  },
});
</script>
...
.cta {
  ...
  svg {
    margin-left: v-bind(spriteMargin);
    min-width: 10%;
    padding: 0;
  }
